...
QString digest(const AuthoritySlotDomain &domain, qint64 generation,
               const QByteArray &payload)
{
    QByteArray input = domain.digestDomain;
...
Parsed parse(const AuthoritySlotDomain &domain, const AuthoritySlotInput &slot)
{
    Parsed parsed;
    if (slot.state != AuthoritySlotReadState::Found) return parsed;
    qint64 generation = 0;
    QByteArray payload;
    if (!AuthoritySlotPublication::parseFrame(
            domain, slot.frame, &generation, &payload)) {
        return parsed;
    }
    parsed.valid = true;
...
AuthoritySlotSelection invalid(const AuthoritySlotDomain &domain,
                               const QString &suffix)
{
    AuthoritySlotSelection selection;
    selection.state = AuthoritySlotSelectionState::Invalid;
    selection.errorCode = domain.errorPrefix + suffix;
    return selection;
}

} // namespace

QByteArray AuthoritySlotPublication::frame(const AuthoritySlotDomain &domain,
                                           qint64 generation,
                                           const QByteArray &payload)
{
    if (!domain.isValid() || generation < 1 || generation > kMaximumGeneration
            || payload.isEmpty()) {
        return {};
    }
    const QJsonObject object{
        {QStringLiteral("schema_version"),
         QString::fromLatin1(domain.frameSchema)},
        {QStringLiteral("generation"), generation},
        {QStringLiteral("payload_base64"), QString::fromLatin1(payload.toBase64())},
        {QStringLiteral("digest"), digest(domain, generation, payload)},
    };
    const QByteArray bytes = QJsonDocument(object).toJson(QJsonDocument::Compact);
    return bytes.size() <= kMaximumFrameBytes ? bytes : QByteArray();
}

bool AuthoritySlotPublication::parseFrame(const AuthoritySlotDomain &domain,
                                          const QByteArray &frameBytes,
                                          qint64 *generation, QByteArray *payload)
{
    if (!domain.isValid() || !generation || !payload || frameBytes.isEmpty()
    ...
    // 模式串必须精确匹配调用方给出的域：一个子系统的槽位字节不能在另一个
    // 子系统里被当作有效授权。
    ...
    if (object.value(QStringLiteral("digest")).toString()
            != digest(domain, parsedGeneration, decoded)) {
        return false;
    }
...
AuthoritySlotSelection AuthoritySlotPublication::select(
    const AuthoritySlotDomain &domain, const AuthoritySlotInput &slotA,
    const AuthoritySlotInput &slotB, const AuthoritySlotInput &legacy)
{
    AuthoritySlotSelection selection;
    if (!domain.isValid()) {
        selection.state = AuthoritySlotSelectionState::Invalid;
        selection.errorCode = QStringLiteral("authority-slot-domain-invalid");
        return selection;
    }
    ...
    if (slotA.state == AuthoritySlotReadState::Unavailable
        ...
        selection.errorCode = domain.errorPrefix + QStringLiteral("unavailable");
        return selection;
    }
    const Parsed parsedA = parse(domain, slotA);
    const Parsed parsedB = parse(domain, slotB);
    ...
    if (slotA.state == AuthoritySlotReadState::Invalid
        ...
        return invalid(domain, QStringLiteral("backend-invalid"));
    }
    if (corruptA && corruptB) {
        // 两个槽位同时损坏无法恢复，但它绝不能退化成"没有授权"。
        return invalid(domain, QStringLiteral("both-corrupt"));
    }
    if (parsedA.valid && parsedB.valid && parsedA.generation == parsedB.generation
            && parsedA.payload != parsedB.payload) {
        return invalid(domain, QStringLiteral("generation-conflict"));
    }
    ...
    if (!newest) {
        if (legacy.state == AuthoritySlotReadState::Invalid) {
            return invalid(domain, QStringLiteral("legacy-invalid"));
        }
