...
void AgentWorkbenchWindow::loadWorkbenchBundle()
{
    QString html = "<!DOCTYPE html><html><head><meta charset=\"UTF-8\">"
        ...
        "const avatars={'user':'👤','agent':'🤖','command':'⚡','usage':'📊','error':'❌','approval':'✋','question':'❓','plan':'📋'};"
        ...
        "html+='</div>';div.innerHTML=html;tl.appendChild(div);tl.scrollTop=tl.scrollHeight;}"
        "function updateTimelineItem(id,delta){const item=document.querySelector('[data-item-id=\"'+id+'\"]');if(!item)return;"
        "if(delta.state){item.className=item.className.replace(/\\b(streaming|complete|cancelled)\\b/g,'');item.classList.add(delta.state);}"
