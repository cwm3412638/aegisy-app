...
- 内置类似 ChatGPT、Claude 的 AI 对话工作区，可切换 API Key 和该 Key 支持的模型，支持流式回复、停止生成和多会话切换
...
12. 点击侧栏“AI 对话”，选择 API Key 和模型后开始流式对话；聊天会话仅保存在当前运行内存中。
...
- AI 对话直接使用所选 API Key 调用兼容接口，客户端不把提示词和回复写入日志或本地文件。
