...
## Restore Approval Controller (2026-09-05)

- 审批策略能判定、审计链能记录，但两者之间没有任何东西拥有顺序纪律：调用方本可以
  按任意顺序把它们接起来——先记录后判定、相信追加而不是重读、把策略拒绝记成"用户
  拒绝了"。本切片新增 `ExtensionStagingRestoreController`
  （`include/` + `src/` `extension_staging_restore_controller.*`），把链条按复核/
  启用控制器的同构形状串起来：load → 判定（委托
  `ExtensionStagingRestoreApprovalPolicy::evaluate`，绝不重新推导——两份对齐检查
  副本会各自漂移，判定原样透传）→ 翻译为审计条目 → 末尾追加并连同读到的代号 CAS
  提交 → 重新读取。它是被测试的纯组件：不执行恢复、不启用任何东西、没有任何产品
  调用方，写盘只经由审计链的 replace。
- 记录/拒绝/人为拒绝的界线划在"人是否真的回答了一个有效问题"：Authorized 判定记
  录 approved 条目；策略以 `declined` 代号拒绝**且**提示确实可展示（Ready）时记录
  declined 条目——问题被问过并被回答了"不"，条目绑定渲染出的提示逐项字段，不携带
  任何授权；其余一切拒绝（提示不可展示、验证状态不对、任何维度漂移、缺少确认，
  包括对一份本就不可能批准的提示说"不"）都不是决定：零写入，一个字节也不碰账本
  （连读都不读），测试以身份与读/写计数器前后比较证明。把拒绝记成 declined 会把
  "从未问出一个有效问题"伪造成"用户拒绝了"。条目绑定渲染出的提示而不是回传：
  批准的回传已被策略逐项证明与提示字节相等，而人为拒绝的诚实标的物是"被问过的
  问题"本身；警告集合沿用呈现层的固定枚举序，一个逻辑集合只有一个规范字节形。
- 读不出、自相矛盾或被锁定的审计链以存储自己的代号阻止记录，并保持冻结直到调用
  方重新加载（控制器无状态，每次记录都重新 load）：在"决定已记录"的字样下面躺着
  一份读不出的日志，等于把一次篡改表述成历史。并发决定由代号比较并交换裁决：
  冲突以独立的 `extension-restore-audit-store-generation-conflict` 透传报告——不
  静默重试，也不是最后写入者获胜——由调用方重新加载并重新提问；测试用假安全
  存储在控制器的 load 与其提交内部的授权复查之间精确注入一次并发提交驱动这条
  路径，冲突后历史保持代号 1 的那一份，重新加载后的重试成功记录为代号 2。
- 提交之后重新读取而不是相信追加：返回的账本来自重新加载的字节。测试在提交与
  重读之间破坏授权字节，控制器如实报告 `authority-invalid` 失败而不是"已记录"；
  删掉重读守卫后该用例立即失败（破坏测试已还原）。上限边界用存储直接预填到
  `MaxEntries`（1024，编译期常量，不可注入，已在测试注释中说明）后经由控制器
  追加第 1025 条：以 `entries-cap` 拒绝且历史一字节不动。非 UTC 决定时间在任何
  字节落盘前以 `entries-invalid` 拒绝。控制器自己的诊断前缀
  `extension-restore-controller-`，四个代号：store-unavailable（空存储）、
  ledger-unusable（读出状态无代号时的兜底）、store-write-failed、
  store-refresh-failed；其余透传存储与审批代号。批准决定返回的凭据仍是纯数据，
  绑定确切计划身份，今天没有任何东西消费它。
- 门禁证据：新增 `extension_staging_restore_controller`（八个聚焦测试：完整链路
  ——真实临时目录树 → 捕获 → 加密暂存往返 → 清点层取描述 → 读回并计划 → 呈现 →
  逐项对齐批准经控制器记录，唯一条目主体/备份 id/目标根/计划身份/树身份/警告集合/
  决定/决定时间逐项绑定，凭据绑定确切计划身份与树身份，独立加载看到同一份历史；
  人为拒绝被记录且不携带授权、账本身份确实推进；三类策略拒绝（计划漂移、未验证
  备份、对 Refused 提示说"不"）各自零写入且连读都不发生，身份与代号前后一致；
  锁定/损坏/矛盾/空存储四态各自独立阻止记录并在后端恢复健康后解除冻结；CAS
  冲突精确注入与重试成功；提交后破坏授权字节时重读如实失败；1024 条上限边界
  拒绝且历史不动；非 UTC 决定时间拒绝且账本保持 Empty）。破坏测试：删掉提交后
  重读守卫后 `testRereadAfterCommitIsTheAuthority` 立即失败，已还原。
  `product_scope_policy` 新增 pin：控制器边界钉、委派而非复制钉（evaluate 调用
  在场，十二个对齐/确认 token 缺席）、declined-vs-refusal 区分与拒绝零写入的
  有序钉（早退先于任何账本读取）、非 Ready/Empty 阻止记录的有序钉、CAS 提交与
  提交后重读的有序钉、诊断前缀与四个代号、无执行/文件系统/计划构建 token（含
  无第二份展示安全规则）、无产品接线（含 `mcp_config_dialog.cpp`）、CTest 注册。
  既有 pin 全部不变通过。本机串行分段门禁：注册总数 105，分段 1–40 / 41–75 /
  76–105 墙钟 129.20s / 9.81s / 51.12s，104/105 通过；唯一失败仍是
  `agent_runtime_protocol` 的
  `platform_terminal_protocol_supports_interaction_resize_and_exit_status`
  （"terminal did not exit"，已在 LastTest.log 按名确认，与本切片无共享代码，
  延续既有环境性 PTY 判定）。未触碰 agent-runtime，未跑 Rust 门禁。
...
## Extension Backup Browsing (2026-09-05)

- 动机：暂存备份域能被写（MCP 保存前备份）也能被管理（清点/保留期层），但产品里没有
  任何东西能回答"这里到底有哪些备份"——回滚能力可以在无人看见的情况下悄悄消失。本
  切片把扩展中心加上一个只读的"暂存备份"浏览区，钉死这份可见性；它是暂存域的第一个
  只读产品消费方，不新增任何写入路径。
- 表面形状：ExtensionCenterDialog 底部独立一区（主体 / 备份 ID / 创建时间 / 验证状态 /
  说明五列只读表 + 状态条），与披露区、更新区同构。选全局区而不是按记录挂接：暂存主体
  语法（`kind:id`）与注册表记录之间还没有诚实的 1:1 映射（保存接线写的是稳定的单主体
  `mcp:claude-settings`，不是某个注册表 id），按记录挂需要先发明这个映射；全主体清点
  让 `mcp:claude-settings` 的备份至少可浏览。对话框构造函数同步纯渲染，供数走
  MainWindow。
- 异步纪律（与复核工作流逐字同构）：独立线程槽位 `m_extensionBackupThread` + 独立单调
  代号 `m_extensionBackupGeneration`——清点只读不写、与账本写入无 CAS 竞争，因此不占
  复核槽位；worker 内只调 `ExtensionStagingBackupInventory::list`（备份根取自唯一产品
  定义点 `extensionStagingBackupRootPath()`；清点路径按设计不触碰密钥，worker 因此
  不需要密钥来源）；完成投递经 QPointer + 代号双守卫；析构先作废代号再
  requestInterruption + wait。对话框打开（复核快照落成）后立即启动一次清点；重开对话
  框即重新加载。
- 诚实状态规则：损坏备份可见并标注"损坏"、绝不被隐藏（藏起来的损坏备份正是回滚能力
  悄悄消失的方式），无法归类主体的损坏条目显示"（主体无法归类）"占位而不是空白；存储
  退化（Invalid / Unavailable）与读取失败各自冻结成一条明确的非空消息（警示样式，
  明说"浏览已冻结""这不是空清单"），绝不渲染成空清单；真空（Empty）是一句完全不同的
  话（"确认一份备份都没有"）——"没有备份"只在确实一份都没有时说。`mcp:` 主体的说明列
  固定写明"备份单元是整个共享设置文件"：按单个服务器描述会暗示恢复范围也只覆盖那一个
  条目。创建时间经 QLocale 本地化；损坏条目没有可信时间戳时明说"时间未知"。
- 展示安全：备份 id、主体、清单身份全部由暂存域语法绑定——id 与目录名逐字节绑定、
  主体只在语法合法时由清点层填写（损坏条目亦然）、清单身份是从读到的字节重算的固定
  格式（`extension-staging-backup-manifest:sha256:<64hex>`，放悬停提示供审计指认
  "是哪一份"）——均非不可控来源文本；清单/存储诊断仍过固定代码正则
  （`^[a-z0-9][a-z0-9-]{0,95}$`）才上屏，不合形者落成"诊断不可展示"。
- 无动作规则（grant-button 先例：授权无法生效的地方不得出现授权按钮）：这一区没有
  任何动作入口——没有恢复、没有删除、没有立即捕获，连灰掉的都没有，因为一个点不动
  的动作会暗示它即将可用，而恢复执行器不存在。静态信息行明说"恢复操作尚未提供，此处
  没有任何恢复、删除或捕获动作"。对话框元对象里也不存在任何备份动作信号：一个发不出
  去的请求比一个能发出去的请求安全，而这里连发不出去的请求都没有。
- 门禁证据：`extension_center_read_only` 扩展（仍单文件 main 惯例，注册数不变）——
  全主体渲染（mcp + skill 两条目齐全、id 在位）、mcp 整文件语义注记、损坏可见并标注
  + 占位主体 + 时间未知、清单身份提示、计数含损坏数、无恢复动作信息行、刷新完整替换
  旧行、读取中清空并明说、Invalid 与 Unavailable 各自冻结且不含"为空"、真空措辞不同、
  伪造诊断（含 `<b>` 与 `/Users/`）不原样上屏、逐格断言浏览表无任何 cell widget、全
  对话框无任何 objectName 含 Backup 或文本含 恢复/删除/捕获 的按钮、元对象无任何备份
  动作信号。`product_scope_policy` 新增钉：浏览区入口/表格/退化冻结/真空措辞/损坏标注/
  整文件注记/无恢复信息行的在场钉，恢复/删除/裁剪/捕获 token 在对话框头与源里的缺席
  钉，MainWindow 只读清点 requireOrdered（list → 唯一根 → 代号比对 → 槽位登记）与
  析构 join 钉，MainWindow 无 removeVerified/applyRetention/planRetention/恢复控制器
  接线的缺席钉；既有的"清点层无产品调用方"缺席钉收窄为 ToolManager/MainWindow 头/
  工作台（MainWindow 与对话框成为显式允许的只读浏览调用方，写入型 token 仍被独立
  缺席钉挡在门外）。本机串行分段门禁：注册总数 107（浏览用例并入既有
  `extension_center_read_only`，注册数不变），分段 1–40 / 41–75 / 76–95 / 96–102 /
  103–107 墙钟 270.36s / 8.72s / 27.45s / 逐条约 17s / 9.30s，107/107 全部通过；
  一次 76–107 整段跑遇到环境性停滞（windows_packaging_policy 脚本单独跑 <1s 即过），
  拆分后全绿，与本切片无关。`git diff --check` 干净。未触碰 agent-runtime，未跑
  Rust 门禁；未触碰 `generate-image.bat`。
...
## Restore Audit Trail Display (2026-09-06)

- 恢复审计链此前只记不看：决定与结果条目入链但没有任何界面读出，无人能审计的审计链
  等于没有。本切片在扩展中心恢复状态行之下新增只读审计轨迹区（`extensionRestoreAuditTable`
  + `extensionRestoreAuditStatus`，六列：决定时间/决定/主体/备份 ID/已确认警告/执行结果，
  按时间倒序）：结果经计划身份与树身份逐字节匹配关联到决定条目，同一计划多条结果取最后
  追加者，游离结果（无匹配决定）不粘到任何行。批准无结果如实渲染"批准已记录，尚无执行
  记录"——绝不暗示执行成功；Partial 必须渲染混合状态、确切失败点（第 N 条）与回退备份
  id（无恢复前备份时如实说没有）；declined 如实说明不携带授权、不产生执行记录；警告措辞
  与批准对话共用同一 `restoreWarningLabel`，批准时核对的与事后读到的是同一句话。
- 诚实规则与备份浏览区同源更严：Invalid/Unavailable/OutcomeUnknown 一律冻结成明确非空
  消息（"查看已冻结……这不是没有记录"），绝不落成空轨迹；"没有记录"只在两个可区分状态
  说出——从未建立（Empty）与已认证的空（Ready 零条目），措辞不同；条目按构造即已认证
  （MAC 失败整体 Invalid 冻结，绝不过滤成行）；存储诊断过固定代码正则后才上屏，条目字段
  过共享展示安全层（过不了显示占位而非改写）；渲染有界（最多上屏最近 64 条，显式截断
  标记交代"审计链完整保留，截断的只是屏幕"）。
- 零动作：轨迹区没有任何按钮、没有单元格控件、没有新增信号出口（信号集合封闭钉死）——
  它是轨迹，不是控制台；对话框绝不构造或写回账本存储。MainWindow 新增只读 tracked
  worker `startExtensionRestoreAuditListing`：独立线程槽位 + 单调代号 + 析构先作废代号再
  join，两半（安全存储授权 + QSettings 载荷）在 worker 内构造，与提交 worker 同一形状，
  读出结果原样交给对话框（不做任何"没有记录"化）；对话框打开时与每次恢复提交完成后各
  刷新一次。
- 门禁证据：`extension_center_read_only`（新增：决定+结果渲染、批准无结果诚实、declined
  无结果、Partial 混合状态+失败点+回退 id、同计划先 Refused 后 Complete 取最新、游离
  结果不粘行、退化/从未建立/已认证空三态区分、非固定诊断正则门控、64 条截断标记、
  零按钮与封闭信号集合断言）与 `product_scope_policy`（冻结措辞钉、无动作 token 缺席钉、
  只读消费钉、worker 纪律钉、提交后刷新顺序钉）全绿；完整串行门禁 109/109 分三段通过
  （1–40 175.62s / 41–75 8.59s / 76–110 50.45s），`git diff --check` 干净。未触碰
