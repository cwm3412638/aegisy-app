...
- [ ] 0.2 Define and implement the authenticated website-to-local configuration projection without exposing credential values outside secure storage
  - Partial metadata-observation foundation: `aegisy-companion-config-projection/0.1` converts an exact authenticated website Key response into an account-bound, credential-free, non-authorizing snapshot. It contains only hashed website-Key identities, bounded display/group metadata, normalized state, website-credential presence, exact source origin/capture time, and fixed false credential-content/configuration-authority/applied fields. Account identity is domain-separated from the verified website account ID; raw IDs, tokens, Key values, Key fragments, user info, and inferred model metadata are absent. API requests bind auth epoch, exact URL/source origin and account verification, disable redirects/cache, enforce JSON Content-Type and per-page bounds, reject incomplete pagination, publish raw Keys only after projection validation, and retire the accumulator immediately. The last valid QSettings cache is isolated by account identity and remains display-only/offline. Profile schema 6 rejects non-derived SecureStorage refs, and credential hints are now domain-separated fingerprints rather than Key substrings. Focused projection/profile/API/tool tests pass. Keep `0.2` unchecked: raw Keys still reach explicit Key/Connect/Chat/Image dialogs after validation; opaque SecureStorage handles, Profile/ConnectWizard source binding, real model projection, revision/expiry authority, and signed/server-authenticated offline cache remain missing.
