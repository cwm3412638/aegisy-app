...
- [ ] 22.5 Verify sidecar and adapter hashes before launch and integrate their versions into updater compatibility checks
  ...
  - Partial signed-update foundation: `aegisy-update-artifact-set/0.1` verifies a bounded losslessly parsed Ed25519 envelope that binds release/channel/target, the full installer, target Runtime/adapter manifest, and one to 64 exact compatible source artifact sets. Production evaluation now requires an opaque `InstalledArtifactSetAuthority`; its only production factory derives the fixed Windows application directory or macOS internal `Contents/MacOS/AegisyClient` layout from the current application path, rejects caller-selected paths, verifies the exact-name adjacent signed receipt/Manifest/Runtime/adapter graph, and binds canonical directory identities plus canonical path, native file identity, size, and SHA-256 for the application-path target and all four adjacent files. Each hashed read matches the opened handle identity to before/after path observations, Runtime and adapter must be distinct native files, and every candidate evaluation rederives and revalidates the graph, so exact-byte identity replacement fails closed. The scalar tuple and arbitrary-root factory exist only in the test build, and a production-shaped child fixture exercises the public fixed-layout path. Every result keeps download/install authority false. The internal `aegisy-update-progress-record/0.1` Store adds strict `{sequence, artifact-set identity, phase, revision}` continuity, exact retry, conflict/rollback/link/tamper rejection, a local cross-process `QLockFile` plus expected-record-identity CAS boundary, atomic file replacement, and fixed-false authority. It is not integrated or anti-deletion, and it has no transaction spanning its record, a secure high-water anchor, updater/framework state, and signed package identity. `UpdateManager` still consumes neither contract; current-path identity is not loaded-process identity or verified outer-package membership; one shared receipt/candidate key has no reviewed rotation chain; Sparkle resume/deltas remain open; and WinSparkle 0.9.3 exposes no pre-download candidate veto. Keep `22.5` unchecked until those gates and signed clean-platform package evidence are closed.
