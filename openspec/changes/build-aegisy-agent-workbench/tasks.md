...
- [ ] 3.6 Define idempotency semantics for turns, approvals, file writes, Git mutations, and job submission
  ...
  - Implemented schema-v22 complete-source slice: the Store accepts exactly one typed `approval-acknowledgement/0.1`, `file-write-acknowledgement/0.1`, `git-mutation-acknowledgement/0.1`, or `background-job-request/0.1` source and derives the lossy draft internally; it never reconstructs source from a draft or accepts an independent source/draft pair. New `present` source record + reservation + metadata-only internal `mutation.reservation-source-recorded` Session event commit atomically. Exact complete-source retry returns the original graph with zero writes and no Session-sequence movement even under low-space admission or writer contention, while any complete-source drift conflicts even if the derived draft is unchanged. Startup captures candidates, validates, reconciles, and asserts no open row in one `IMMEDIATE` transaction; `present` histories are exactly `[source]` or `[source, reconciliation]`. Migrated v21 rows become `legacy-unavailable` plus `reconciliation-required` revision 2 with exactly empty lifecycle history `[]`. The v21 copy is preceded by exact-schema, 10,000-row, and semantic row validation; all pre-v22 paths reject reserved event-kind/operation-ID/Event-ID namespaces and shared `events`/`session_sequences` Triggers. `user_version` and `application_id` are rechecked under the migration lock; a separately pre-opened read-only connection holds one `DEFERRED` snapshot for WAL-consistent backup, and file-identity checks reject path replacement before the Store becomes writable. A current-v22 database skips migration locking, while a concurrent completed v22 migration is accepted safely. Canonical inventory covers all reservation objects, and the lifecycle lookup is locked to its covering index by an `EXPLAIN QUERY PLAN` regression. This adds no production producer, AAP/Qt surface, Public Timeline event, outcome/result anchor, consume/caller CAS, dispatch, filesystem/Git/job execution, genuine user Approval, or authority; Agent/Codex remains read-only and `3.6` remains unchecked.
  - Implemented terminal-outcome contract slice: `aegisy-agentd/src/mutation_reservation_outcome.rs` accepts only a terminal revision-two approval/file-write/Git acknowledgement or a terminal background-job state, validates it against the exact complete source, and derives bounded canonical bytes, SHA-256, and a source-bound domain-separated outcome identity. Approval outcomes remain non-granting (`denied`, `expired`, `not-required`, or failed); file/Git outcomes are only committed or failed; uncertain and non-terminal states are rejected. Seven focused tests cover all four kinds, canonical round-trip, revision/state gates, source-kind and hidden binding drift, job-request drift, unknown fields, non-canonical JSON, and forged authority. This is still an in-memory crate contract: schema v22 has no durable outcome row/event, caller CAS, consume route, production producer, AAP/Qt surface, dispatch, filesystem/Git/job execution, or genuine user Approval, so `3.6` remains unchecked.
  - Partial file-write foundation: `aegisy-agentd/src/file_write_ack.rs` defines a metadata-only `file-write-acknowledgement/0.1` contract. Its operation identity binds Session/project/root, idempotency key, request fingerprint, and Workspace Edit identity; Accepted/Committed/Failed/ReconciliationRequired revisions are contiguous and monotonic, uncertain state cannot be resolved by the producer, terminal observations require an opaque hash, and mutation/execution authority are fixed false. Its complete typed source can enter only the internal v22 reservation graph, which derives the draft itself; no production producer, AAP/Qt route, outcome acknowledgement/consume path, filesystem write, Approval, or user-decision path exists. Approval, Git, and job producers remain absent.
...
- [ ] 5.1 Define SQLite schema for projects, roots, sessions, lineage, turns, items, jobs, approvals, extensions, model profiles, and Git checkpoints
  - Partial schema foundation: Schema v22 currently verifies 33 required tables,
    ...
    mutation_reservation_sources, and model profiles, with bounded constraints and
    indexes for the implemented domains.
  - Implemented schema-v22 non-Turn source-record slice adds immutable `present`
  ...
  - Remaining: Extensions table, Git checkpoint projections, complete scheduler recovery, and the production non-Turn acknowledgement/outcome schema.
- [ ] 5.2 Implement append-only event persistence with monotonic sequence and transaction boundary before mutation acknowledgement
  ...
  - Implemented v22 event slice commits a new `present` source record, derived
  ...
  - Remaining: production producer-side acknowledgement, result/outcome anchors,
    consumption/caller CAS, and reviewed recovery for approvals, files, Git, and
    jobs. The internal v22 metadata event graph is not a completed acknowledgement
    or product recovery stream.
