...
- [ ] 3.6 Define idempotency semantics for turns, approvals, file writes, Git mutations, and job submission
  ...
  - Shared reservation-design foundation: `aegisy-agentd/src/mutation_reservation.rs` converts the existing metadata-only approval, file-write, Git-mutation, and background-job request contracts into one strict `mutation-reservation-draft/0.1`. It normalizes the future ledger key and fingerprint shapes and classifies exact retry, same Session/kind/key conflict, and unrelated requests. Every draft explicitly reports schema-v20/Turn-anchor incompatibility, non-persistence, and fixed-false dispatch/mutation/approval/execution authority. This prevents direct insertion into the current `mutation_kind = 'turn-start'` table; a reviewed schema migration, source-record binding, per-kind anchors, Store transactions, recovery, AAP, and Qt consumption remain required.
  - Partial file-write foundation: `aegisy-agentd/src/file_write_ack.rs` defines a metadata-only `file-write-acknowledgement/0.1` contract. Its operation identity binds Session/project/root, idempotency key, request fingerprint, and Workspace Edit identity; Accepted/Committed/Failed/ReconciliationRequired revisions are contiguous and monotonic, uncertain state cannot be resolved by the producer, terminal observations require an opaque hash, and mutation/execution authority are fixed false. The module is not connected to AAP, Store, Qt, filesystem writes, approvals, or a user decision; approval, Git, and job producers remain absent.
...
- [ ] 17.3 Implement pinned context for files, selections, images, diagnostics, terminal excerpts, Git commits/diffs, artifacts, and child handoffs
  ...
  - `artifact/read-command-output` now returns the originating `session_id` as an additive binding field. The Qt read-only Artifact dialog exposes an explicit `固定完整输出` action only for valid UTF-8 text with matching reference/SHA-256 and the active project-bound Work session; it persists a session-owned `artifact` descriptor through the existing pinned-context CAS with `metadata.item_id`, priority 700, and retained UTF-8 byte count. Cross-session, blocked/recovery, busy-mutation, invalid-identity, and unsupported-media states are visible but disabled/fail closed; the action never sends the Artifact implicitly.
...
- [ ] 22.5 Verify sidecar and adapter hashes before launch and integrate their versions into updater compatibility checks
  - Partial local foundation: `aegisy-artifact-manifest/0.1` validates a present sidecar/adapter manifest before Qt launches `aegisy-agentd`, including bounded manifest/artifact sizes, fixed runtime/adapter identities, relative-path and symlink-escape denial, version metadata, and SHA-256 verification. Focused CTest coverage proves valid, tampered, path-traversal, and unknown-field rejection. Developer builds may omit the manifest for compatibility; release packaging must still generate it, Rust adapter launch and updater compatibility must consume the same trusted manifest, and Windows signed-artifact evidence remains open.
