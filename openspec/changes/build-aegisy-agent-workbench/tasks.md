...
- [ ] 3.6 Define idempotency semantics for turns, approvals, file writes, Git mutations, and job submission
  ...
  - Shared reservation Store foundation: `aegisy-agentd/src/mutation_reservation.rs` converts the existing metadata-only approval, file-write, Git-mutation, and background-job request contracts into strict `mutation-reservation-draft/0.1` values. Workbench database schema v21 persists a validated draft plus redundant identity/scope columns and its SHA-256 in a separate `mutation_reservation_records` table; the schema-v20 Turn ledger and AAP wire contract are unchanged. The unique Session/kind/idempotency tuple reuses an exact `reserved` retry through read-only preflight without write admission, including under low space or a competing writer. For an absent key, the `IMMEDIATE` transaction rechecks the database-backed Session deletion state, Session status, project/root/Turn scope, and idempotency tuple before insertion. A deletion committed between preflight and that transaction returns `session-deletion-pending` and leaves no reservation row. Fingerprint or source-binding drift conflicts, and different keys and kinds remain isolated. Startup advances every unresolved `reserved` row exactly once to `reconciliation-required`; another reservation attempt then fails closed. The inner draft intentionally keeps `reservation_persisted`, schema-v20/Turn-anchor compatibility, and dispatch/mutation/Approval/execution authority false: only the validated v21 wrapper is persistence evidence, never authority. The Store enforces Session/project/root/Turn scope and foreign keys, root-removal protection, Session-owned reads, pending-deletion retry denial, a 10,000-row global limit, a 16 KiB draft bound, startup integrity scanning, Session purge, and v20-to-v21 migration with no fabricated row. Every `<21` migration rejects and rolls back if the reservation table or either named index already exists, including case-insensitive View/index-only collisions and exact valid v21 data. Schema-v21 startup compares the complete table-bound `sqlite_master` inventory with a freshly materialized canonical schema, so extra Trigger/index objects fail closed before reconciliation. No production approval/file/Git/job producer calls this internal API; no Session event, result anchor, consume path, AAP/Qt surface, dispatch, or mutation authority exists, so `3.6` remains unchecked.
  - Partial file-write foundation: `aegisy-agentd/src/file_write_ack.rs` defines a metadata-only `file-write-acknowledgement/0.1` contract. Its operation identity binds Session/project/root, idempotency key, request fingerprint, and Workspace Edit identity; Accepted/Committed/Failed/ReconciliationRequired revisions are contiguous and monotonic, uncertain state cannot be resolved by the producer, terminal observations require an opaque hash, and mutation/execution authority are fixed false. Its metadata draft can enter only the internal v21 reservation wrapper; no production producer, AAP/Qt route, outcome acknowledgement/consume path, filesystem write, Approval, or user-decision path exists. Approval, Git, and job producers remain absent.
...
- [ ] 5.1 Define SQLite schema for projects, roots, sessions, lineage, turns, items, jobs, approvals, extensions, model profiles, and Git checkpoints
  - Partial schema foundation: Schema v21 currently verifies 32 required tables,
    including projects, project_roots, sessions, turns, items, background_jobs,
    mutation_acknowledgements, mutation_reservation_records, and model profiles,
...
- [ ] 5.2 Implement append-only event persistence with monotonic sequence and transaction boundary before mutation acknowledgement
  ...
  - Remaining: producer-side acknowledgement Session events, result anchors,
    consumption/CAS, and recovery for approvals, files, Git, and jobs; the v21
    reservation-only table has no Session event or outcome and is not a completed
