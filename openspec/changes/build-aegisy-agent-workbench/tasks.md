...
- [ ] 0.3 Complete one-click profile apply/repair with target preview, per-tool isolation, backup, atomic verification, and truthful rollback status
  ...
  - Native macOS evidence follow-up: a fresh `--depth 1` clone was configured out of tree and driven with exactly the two commands `.github/workflows/macos-build.yml` runs and `macos_ci_policy` pins verbatim, `cmake --build build -j4` over the complete target graph and unfiltered `ctest --test-dir build --no-tests=error --output-on-failure`. Both returned exit 0 with zero compiler errors, zero warnings, and `59/59` passing in 267.84 seconds; a separate fresh configure confirmed all 59 tests register from a clean tree. The produced `AegisyClient.app` carries the executable, `Info.plist`, embedded `Sparkle.framework`, bundled `workbench` resources, and the `aegisy-agentd` sidecar. Configuring a clean clone needs network egress for the pinned Sparkle 2.9.4 archive that this sandbox cannot reach; seeding it from cache is hash-equivalent because `cmake/Sparkle.cmake` re-verifies the pinned SHA-256 and deletes on mismatch, so this is an environment limitation rather than a repository defect. Not verified: the bundle is adhoc/linker-signed with no sealed resources so it is not a notarized distributable, and the Windows one-click path cannot be exercised from macOS with `gh` unauthenticated. `0.3` stays unchecked because Windows native evidence remains open.
...
- [ ] 0.4 Build one extension center for compatible Codex plugin enablement, custom Skills management, and MCP, including provenance, compatibility, import, enable/disable, update, removal, and recovery
  ...
  - Evidence-based compatibility follow-up: `ExtensionCompatibilityPolicy` replaces the blanket source-asserted `Unknown` with one decision made from checkable host evidence. Source adapters now report facts only and the coordinator applies the policy once before registry revalidation. The granted capability set is fixed to `filesystem-read`/`mcp-tools`/`network`/`skill-content`, excluding `process` and every write capability to match read-only Agent/Codex authority, and the Codex host version comes only from the locally detected `ToolManager` version. A requested capability outside the granted set is a definite `Incompatible` decided before any unknown-evidence fallback, so unreadable versions and missing host evidence cannot downgrade a rejection. Missing or corrupt evidence never resolves to `Compatible` and reports distinct `codex-plugin-host-version-unknown`, `codex-plugin-host-version-unreadable`, and `codex-plugin-version-missing` codes; no minimum Codex version is invented because none is pinned in the repository. `apply()` writes only compatibility state and reason and leaves trust, effective enablement, update, and recovery untouched, so the registry's `Verified + Compatible` gate is unchanged and nothing became enableable. Focused policy, coordinator, source, registry, and product-scope tests pass, and the full serial gate is `60/60`. Import, enable/disable, update, removal, backup, and recovery workflows remain open, so `0.4` stays unchecked.
  - Reviewed trust follow-up: `ExtensionTrustPolicy` supplies the other half of the registry's `Verified + Compatible` gate. Trust comes only from a review pin binding kind, id, source identity, and content identity jointly, so a review applies to exact content rather than a name; content or source drift revokes it with distinct codes. Fail-closed rules dominate: duplicate or conflicting pins for one `(kind, id)` yield `extension-review-conflict` in either ordering rather than selecting the matching pin, one malformed pin fails the whole evaluation instead of being skipped, an oversized store is rejected rather than truncated, and a record with invalid id or identities is rejected before matching. `apply()` writes only trust and leaves compatibility, effective enablement, update, and recovery untouched, so a `Verified + Compatible` record passes registry validation while still not enabled and an enabled record loses validity when trust is revoked. No review evidence exists in the product: `MainWindow` supplies no pins and product policy asserts it stays that way, so every shipping record remains `Unverified` and unenableable. Focused trust, coordinator, registry, source, and product-scope tests pass, and the full serial gate is `61/61`. Import, enable/disable, update, removal, backup, and recovery workflows remain open, so `0.4` stays unchecked.
  - Authenticated review ledger follow-up: `ExtensionReviewLedger` gives review pins an authenticated home, because the alternative — an ordinary editable review file — would let anyone grant extension trust by editing text. It follows the companion activation journal conventions: schema `aegisy-extension-review-ledger/0.1`, domain-separated HMAC-SHA256 under a required 32-byte key, 8-byte big-endian length-prefix framing, `CRYPTO_memcmp` comparison, `OPENSSL_cleanse`, exact key-set JSON validation, and a bounded monotonic generation. The MAC covers the generation, the pin count, and every field of every pin in order, so appending, removing, reordering, or substituting a pin's content identity all fail as `extension-review-ledger-mac-mismatch` rather than being undetectable. `Empty` is reachable only from genuinely empty input; corruption reports `Invalid` with distinct `-oversized`/`-record-invalid`/`-pin-limit`/`-pin-invalid`/`-pin-duplicate`/`-mac-mismatch` codes and an unusable key reports `Unavailable` with `-key-unavailable`, so tampering never degrades into a clean slate. An authenticated empty set is a valid "reviewed, nothing trusted" state distinguishable from an absent ledger, and it grants no trust. Authentication completes before any pin is returned, so a failed ledger yields no usable reviews. The layer has no persistence, no enablement authority, and no trust decision of its own, and `MainWindow` never loads a ledger, so every shipping record remains `Unverified` and unenableable. Focused ledger and product-scope tests pass and the full serial gate is `62/62` in 203.43s; a first attempt run concurrently with a build failed `agent_workbench_render` at the real-`clangd` `AWB_EDITOR_LSP` stage and passed standalone and clean, and that investigation surfaced an unrelated `QString::arg` defect in `finishPinnedDiagnosticRaw` that silently drops severity from the pinned diagnostic identity, recorded for its own slice. A human review workflow producing pins, a persistence adapter, and every import/enable/disable/update/remove/backup/recovery workflow remain open, so `0.4` stays unchecked.
  - Split review persistence follow-up: `ExtensionReviewLedgerStore` gives the authenticated ledger somewhere to live, splitting persistence the way the companion activation journal does — authority (the 32-byte HMAC key plus the committed generation and identity) in platform secure storage behind an injected `ExtensionReviewLedgerSecureStore`, payload bytes in `QSettings` under `extensions/review-ledger/record`. The key never enters ordinary settings and the pins are never copied into the authority, asserted against the actual persisted bytes, and key material is cleansed on every path. Anti-degradation is the load-bearing property: deleting either half is a distinct failure rather than a quiet return to "never reviewed", so an orphaned payload reports `extension-review-store-record-without-authority`, an orphaned authority reports `-record-deleted`, a corrupt payload surfaces the ledger's own `extension-review-ledger-*` code, a locked backend resolves to `Unavailable` with no pins, and `Empty` is reachable only when both halves are genuinely absent. Replay is closed: an old payload was legitimately signed once, so the authority pins the committed generation and identity and a mismatch becomes `-record-superseded` instead of reviving revoked reviews. Publication is three-phase (reserve then write payload then commit) so an interrupted write is adjudicated by what landed on disk rather than inferred — a reservation matching the on-disk identity is promoted, a non-matching one is rolled back and cleared, and a resolution that cannot itself be persisted yields `OutcomeUnknown` with no pins and further commits blocked until the backend recovers. Concurrent reviews are resolved by compare-and-set on the generation rather than last-writer-wins, and malformed or duplicate pins are rejected before anything is written so a failed commit leaves no reservation residue. All three interruption points are driven through an injectable fake secure store and a temporary `QSettings` root, including recovery being persisted rather than re-inferred on every read. The store grants no `effectiveEnabled`, decides no trust, and `MainWindow` names neither the ledger nor the store, so every shipping record remains `Unverified` and unenableable. Focused store and product-scope tests pass and the full serial gate is `63/63` in 596.74s. A `SecureStorage`-backed adapter, a human review workflow producing pins, and every import/enable/disable/update/remove/backup/recovery workflow remain open, so `0.4` stays unchecked.
  - Review authority anchor follow-up: the review ledger's HMAC key exists in no other location, so a single torn secure-storage write would leave every stored review permanently unauthenticatable — the same hazard the companion activation journal already solved with A/B slot publication. Rather than duplicate that recovery logic into a second copy that can drift, it is extracted to `AuthoritySlotPublication` and parameterized by a caller-supplied domain; `CompanionActivationAuthoritySlots` becomes a facade holding only its own two domain constants. Domain separation is the new security property: the frame schema and digest domain both enter the persisted bytes, so a slot frame from one subsystem fails to parse in the other and relabelling `schema_version` does not help, error codes carry a per-domain prefix, and an unconfigured domain is rejected rather than falling back to a default format. The extraction is byte-compatible by requirement, and the focused test recomputes the companion digest independently from the domain string, 8-byte big-endian length prefixes, and generation-then-payload ordering so implementation drift is caught instead of mirrored; the companion's own slot and journal tests pass unchanged, alongside coverage of first install, peer-slot targeting, unavailable backends, same-generation conflicts, corruption recovered from the peer, both-corrupt, corrupt-without-peer, generation exhaustion, and the retained legacy migration path. `SecureStorageExtensionReviewLedgerAdapter` then anchors review authority under `extensions/review-ledger-authority/slot-{a,b}/v1` through `loadEncryptedFresh`, so a locked backend is never misread as "never reviewed"; it deliberately has no legacy single-slot migration path because the subsystem is new and adopting a stray envelope would only be an attack surface, and it moves bytes only — it never parses pins, decides trust, or grants enablement, and an unconfirmable write returns `OutcomeUnknown` for the store to resolve. Focused publication, companion, store, and product-scope tests pass and the full serial gate is `64/64` in 195.91s. Nothing in the product path creates, reads, or publishes review authority, so every shipping record remains `Unverified`; a human review workflow producing pins and every import/enable/disable/update/remove/backup/recovery workflow remain open, so `0.4` stays unchecked.
  - Human review planning follow-up: `ExtensionReviewWorkflow` turns an approve or revoke action into the complete review set that should exist after the commit, rather than mutating storage in place, so every security property of approval and revocation is decidable without persisting anything and `ExtensionReviewLedgerStore` keeps sole authority over how a set lands on disk; the plan echoes the ledger generation so the store compares and swaps instead of overwriting a concurrent review. Approval binds to the exact content the human saw: the request carries the source and content identities rendered at review time, and a mismatch fails as `extension-review-content-drift` or `-source-drift` instead of re-targeting the decision onto whatever is on disk now. Absent, duplicated, uninstalled, and self-inconsistent records are rejected with distinct codes, because approving an absent record would pre-authorize content that has not appeared yet and a duplicated `(kind, id)` means the inventory is no longer trustworthy enough to pick one from. The stored set is adjudicated before it becomes a base: `Invalid`, `Unavailable`, and `OutcomeUnknown` all reject with `-ledger-unusable` since committing a partial set as a complete one would silently delete the reviews that failed to load, a malformed existing pin rejects rather than being laundered into authenticated evidence, an already-conflicting set rejects because "which duplicate to drop" has no correct answer, and an `Empty` result carrying pins or a non-zero generation is self-contradictory. Re-approval replaces the pin for the same `(kind, id)` rather than appending a second one that would manufacture exactly the conflict the trust policy must reject, revocation matches on `(kind, id)` alone so a tampered or already-removed extension can still be revoked instead of being stranded permanently, and a full set rejects new approvals with `extension-review-pin-limit` instead of evicting an existing review while revocation still shrinks it. Focused workflow and product-scope tests pass, the guards were confirmed to fail when the drift comparison, the replace-in-place branch, or the unusable-ledger rejection is removed, and the full serial gate is `65/65` in 198.18s (run with a clean environment: exporting `AEGISY_AGENT_BACKEND=preview` for the whole `ctest` run leaks into `cargo test` and produces 22 spurious `agent_runtime_protocol` failures). Nothing in the product path calls the workflow, so every shipping record remains `Unverified` and unenableable; wiring review into the extension center and every import/enable/disable/update/remove/backup/recovery workflow remain open, so `0.4` stays unchecked.
  - Review presentation follow-up: a human review decision is only as trustworthy as what was rendered, and extension names, versions, scopes, and capability lists all come from untrusted disk sources, so `ExtensionReviewPresentation` decides whether a record can be displayed at all before anyone is asked to approve it; it renders only, approving nothing, persisting nothing, deciding no trust, and granting no enablement. Unpresentable text is rejected rather than cleaned or truncated, because truncation would let two different extensions render identically and sanitizing would display a name that does not exist: control, format, surrogate, private-use, and unassigned categories are refused along with the bidirectional overrides and isolates, zero-width characters, line/paragraph separators, and BOM that fall outside those categories but still make the screen disagree with the underlying string, and leading or trailing whitespace is refused for the same reason. The prompt echoes the exact full source and content identities it displayed and `ExtensionReviewWorkflow` compares against them, so drift between rendering and approval is detected instead of silently re-targeted, while the short fingerprint is display-only and preserves both ends of the digest since showing a prefix alone would let a constructed prefix collision look identical on screen. Risks are emitted as explicit warnings in a fixed order rather than left to layout — a name unrelated to its identifier, an unknown version, a capability outside the granted set, a write or execution capability against the read-only boundary, unresolved compatibility, and content changed since the last review — and duplicate or over-limit capability lists, unsafe capability names, invalid identifiers or digests, uninstalled targets, and a claimed prior review without a usable previous digest all reject outright. Focused presentation and product-scope tests pass, the guards were confirmed to fail when the invisible-character range is dropped, when an over-long name is truncated before the safety check, or when the fingerprint is echoed instead of the full identity, and the full serial gate is `66/66` in 266.79s. Nothing in the product path builds a review prompt, so every shipping record remains `Unverified` and unenableable; wiring review into the extension center and every import/enable/disable/update/remove/backup/recovery workflow remain open, so `0.4` stays unchecked.
  - Product review workflow follow-up: Extension Center now loads the authenticated review ledger through its SecureStorage A/B authority and QSettings payload on a tracked worker, and explicitly freezes review actions for Invalid/Unavailable/OutcomeUnknown instead of treating them as an empty set. It shows scope, requested capabilities, trust, compatibility, review actions, and stale review pins whose source disappeared. Approve uses the spoof-resistant PlainText prompt with complete source/content identities and a default-unchecked confirmation; revoke can remove a stale or drifted pin by `(kind,id)`. `ExtensionReviewController` then reloads the ledger and all three sources, runs `ExtensionReviewWorkflow::plan`, uses generation CAS to replace the complete pin set, and full-rescans with returned pins before trust can become Verified. Source/content drift after presentation, unusable ledger, invalid authority, and no-op revocation are zero-write. Failed review operations remain frozen until reload. Worker callbacks bind a monotonic operation generation and MainWindow joins an active worker on destruction. Review UI exposes no install/enable/update/remove/execute action and Verified never changes `effectiveEnabled`. Focused controller/dialog/workflow/ledger/product tests pass and the full serial gate is `67/67` in 781.74s. `0.4` remains unchecked for reviewed enable/disable, import, update, removal, encrypted backup, rollback, and recovery workflows.
  - Reviewed enablement follow-up: `ExtensionEnablementPolicy` supplies the independent enable action that the registry's `Verified + Compatible` gate has always required but that nothing produced, because review only answers "a human saw and accepted this content" and compatibility only answers "the current host grant can hold it" — neither means the user asked for it to run. A grant binds `(kind, id, sourceIdentity, contentIdentity)` jointly exactly as a review pin does, so replacing content cannot inherit the previous content's enablement, which is the precise hazard enablement authority exists to prevent; content drift is reported ahead of source drift because once the content changed the source no longer needs distinguishing. Fail-closed rules match the trust policy: duplicate or conflicting grants for one `(kind, id)` yield `extension-enablement-conflict` in either ordering rather than selecting the matching one, a single malformed grant fails the whole evaluation instead of being skipped, an oversized store is rejected rather than truncated, and an unverifiable record is refused before matching. A valid grant cannot bypass the other gates: uninstalled, unreviewed, and unknown/incompatible records each refuse with a distinct code while a missing grant still reports `extension-not-enabled`, so diagnostics never describe a missing grant as a missing review, and write/execution capabilities remain outside the granted set so extensions requesting them stay definitely `Incompatible` and can never pass this layer. `apply()` writes only `effectiveEnabled` and leaves trust and compatibility untouched, keeping the registry gate an independent second check — the focused test proves a granted eligible record passes registry validation, that revoking trust makes the registry reject the same enablement, and that re-evaluation withdraws it — and the layer reaches no `QProcess`, `QFile`, or `QSettings`. Nothing in the product path supplies grants: `MainWindow`, the Extension Center dialog, and the inventory coordinator all name no enablement policy and product scope pins that absence, so every shipping record remains unenabled. Guards were confirmed by sabotage to fail when the conflict rejection, the content-drift comparison, or the trust gate is removed, and the full serial gate is `68/68` in 201.13s. Enablement now has no persistence, no UI, and no grant producer, and import, update, removal, encrypted backup, rollback, and recovery remain open, so `0.4` stays unchecked.
  - Shared evidence ledger follow-up: enablement grants need the same authenticated home review pins have, because an ordinary editable file would let anyone grant extension enablement by editing text, and the grant record shape is identical to a review pin — so duplicating the ledger would have created two copies that can drift apart, exactly the hazard the A/B slot extraction already resolved. `ExtensionEvidenceLedger` therefore holds the codec (schema/generation/entries/mac framing, domain-separated HMAC-SHA256 under a required 32-byte key, 8-byte big-endian length prefixes, `CRYPTO_memcmp` comparison, `OPENSSL_cleanse`, exact key-set JSON validation, bounded monotonic generation) while `ExtensionReviewLedger` and `ExtensionEnablementLedger` become thin facades holding only their own domain constants. Domain separation is the security property rather than a formatting detail: review evidence and enablement grants are two different authorizations, and a shared format would let a review payload's bytes be moved into the grant position, turning "a human saw this content" into "a human asked to run this content" — schema, MAC domain, and identity domain all differ so a payload from one fails to parse in the other, relabelling `schema` does not help because the MAC domain also participates, and an unconfigured domain is rejected outright instead of falling back to a default format. The extraction had to be byte-compatible or existing installs would stop reading their own review evidence, so `extension_review_ledger` now recomputes the review MAC and identity independently from the domain string, length prefixes, and generation-then-set ordering, catching implementation drift instead of mirroring it, and the review ledger/store/workflow/presentation/controller tests pass unchanged while entry-level diagnostic codes keep each caller's own noun (`pin` / `grant`) so no previously pinned code changed. `extension_enablement_ledger` covers round trip, an authenticated empty set distinguished from an absent payload, key substitution and unusable keys, append/remove/reorder/substitute and generation tampering all reporting `-mac-mismatch`, malformed and duplicate grants, cross-domain parse failure in both directions, the relabelled-review-payload attempt, and independent wire recomputation; product scope pins moved to the shared layer along with the logic. The ledger carries evidence only — a parsed grant still goes through `ExtensionEnablementPolicy`, which additionally requires reviewed, compatible, and installed. Guards were confirmed by sabotage: collapsing the enablement domains onto the review domains makes a relabelled review payload parse as a grant set, and changing the review MAC domain fails the byte-compatibility check. The full serial gate is `69/69` in 192.94s. Nothing in the product path reads or writes grant payloads, so every shipping record stays unenabled; grant persistence, a grant-producing UI action, and every import/update/remove/backup/recovery workflow remain open, so `0.4` stays unchecked.
  - Shared evidence persistence follow-up: grant payloads need the same split-persistence home review pins have — authority (the HMAC key plus the committed generation and identity) in platform secure storage, payload bytes in `QSettings` — and the store logic was already proven for review evidence, so duplicating it would have created two copies of the three-phase publication that can drift apart. `ExtensionEvidenceLedgerStore` therefore holds the reserve/write-payload/commit sequence, the anti-degradation adjudication, and the generation compare-and-set, while `ExtensionReviewLedgerStore` and `ExtensionEnablementLedgerStore` become facades holding only their own authority schema, `QSettings` record key, diagnostic prefix, and ledger domain plus an adapter forwarding their injected secure store; an unconfigured domain is rejected in both `load()` and `replace()` rather than falling back to a default format. Anti-degradation matters even though degrading to empty is the safe direction here: no grant means no enablement, but reporting a deleted half as "never granted" would describe a tampered record as a user who never asked to enable anything, so an orphaned payload, a deleted payload, a corrupt payload, and a locked backend each report their own distinct failure. Persistence-level domain separation is the security property: a review authority envelope and payload moved wholesale into the enablement position is rejected as `extension-enablement-store-authority-invalid`, and the two payload keys differ so neither can overwrite the other, closing the path from "a human saw this content" to "a human asked to run this content". Every previously pinned diagnostic code survived because the entries noun is a domain field (`entriesCodeNoun`), so the review store still emits `extension-review-store-pins-invalid` while the grant store emits `extension-enablement-store-grants-invalid`. Replay is rejected by anchoring on the committed generation *and* identity, since an old payload was legitimately signed once and restoring it would otherwise revive revoked enablement. `extension_enablement_ledger_store` covers empty/first write, both orphaned-half codes plus corrupt payload and locked backend, replay after revocation, all three interruption points through an injectable fake secure store, resolution being persisted rather than re-inferred, generation CAS with stale/negative/invalid inputs rejected before any reservation lands, and cross-domain rejection at the persistence level with the review evidence left undamaged. Guards confirmed by sabotage: sharing the review authority schema makes a review envelope adoptable as enablement authority, sharing the record key makes review evidence read as grants, disabling the superseded check revives a revoked grant set, blanking the orphan code degrades a half-deleted record to empty, and both generation CAS sites must be intact for a stale write to be refused. The full serial gate is `70/70` in 227.41s. Nothing in the product path constructs an enablement store and `product_scope_policy` pins that absence for the main window, the extension center, and the review controller, so every shipping record stays unenabled; a grant-producing UI action, import, update, removal, encrypted backup, rollback, and recovery remain open, so `0.4` stays unchecked.
  - Enablement grant planning follow-up: the grant store could hold grants but nothing could produce one, so `ExtensionEnablementWorkflow` translates an explicit human enable/disable request into the complete grant set that should exist after the commit plus the generation to compare-and-set on, mirroring `ExtensionReviewWorkflow` so every security property is decidable and testable with no persistence in the layer at all. Planning requires reviewed *and* compatible *and* installed at plan time rather than deferring to evaluation: evaluation independently requires all three so a premature grant would never enable anything today, but it would sit in the ledger as authenticated authority and take effect the moment review appeared, which is pre-authorizing future content, so the plan refuses it outright with `extension-enablement-trust-missing` / `extension-enablement-compatibility-missing` / `extension-enablement-target-not-installed`. The plan is always the complete set and never a delta, and planning against an unreadable ledger is refused as `extension-enablement-ledger-unusable` because committing a partial set would silently revoke the grants that failed to load — a safe direction that nevertheless reports a tamper as a user who chose to disable and commits under the wrong generation. Revocation is keyed on `(kind, id)` alone so content drift, a vanished record, a revoked review, and an incompatible record all remain revocable (otherwise a tampered extension could never have its enablement authority withdrawn) while not matching across kinds; an invalid grant already in the ledger fails the whole plan rather than riding along, since committing it would authenticate the set and launder that entry into valid authority; re-granting is replace rather than append so the set never reaches the conflicting state the policy must reject, and an identical re-grant reports `changed == false` so no generation is burned. `extension_enablement_workflow` covers granting, set preservation, idempotent and updated re-grants, revocation under drift/absence/revoked-review, kind-scoped revocation, both drift codes, every missing gate including unknown compatibility, absent/ambiguous/unverifiable targets, malformed request and disable IDs, all three unusable ledger states for both actions, contradictory empty and negative generations, laundering and conflicting existing sets, the grant limit with replacement still allowed at the limit, and policy agreement showing a planned grant enabling an eligible record yet not surviving review revocation or content drift. Guards were confirmed by sabotage with `Building CXX` observed each time: dropping the trust gate, the compatibility gate, either drift comparison, or the unusable-ledger check; planning a delta instead of the full set; skipping the existing-invalid-grant rejection; matching revocation across kinds; and appending instead of replacing. The full serial gate is `71/71` in 195.19s. The workflow has no caller and `product_scope_policy` pins that the main window, the extension center, and the review controller neither plan nor persist grants, so every shipping record stays unenabled; the UI action that raises a request, import, update, removal, encrypted backup, rollback, and recovery remain open, so `0.4` stays unchecked.
  - Enablement grant control follow-up: `ExtensionEnablementController` binds the grant workflow to the grant store — load, plan, commit through generation CAS, then re-read — structurally identical to `ExtensionReviewController`. The authority boundary is the whole point of the slice: the controller does **not** feed grants into `ExtensionInventoryCoordinator`, because the coordinator would write `record.effectiveEnabled`, which is the authority to actually run extension content, and that gate must stay shut until the permission, approval, sandbox, and recovery gates exist. Instead the enablement decision is returned as a positional projection (`decisions`, parallel to `inventory.records`) for display and diagnostics only and is never written back, so a committed grant evaluates to "should be enabled" while the record itself stays unenabled and the registry's `Verified + Compatible` gate is untouched. After a commit the controller re-reads rather than trusting the plan, since only the bytes read back are the authority that actually took effect; an unreadable ledger yields no grants and blocks writes, because showing "these extensions were never granted" when the current grants are unknown would misreport a tamper as a user choice. `extension_enablement_controller` covers the empty-and-ungranted initial snapshot with a decision projection matching the record list, refusal to grant an unreviewed record without leaving authenticated authority behind, a reviewed and compatible record still not enabled without a grant, content drift between presentation and grant writing nothing, a successful grant refreshing the projection while leaving `effectiveEnabled` false, an idempotent re-grant not advancing the generation, a grant surviving in the ledger yet not enabling after review revocation (`TrustMissing`), revocation under content drift and after the source disappears, a no-op revocation not advancing the generation, invalid and locked authority blocking both inspection downgrade and writes, and a null store reporting unavailable rather than empty. A fixture lesson is recorded: a Skills record requests `filesystem-read`, so with an empty `host.grantedCapabilities` the compatibility policy decides `Incompatible` via `extension-capability-not-granted` and a grant request never reaches the gates under test, so the fixture must set `ExtensionCompatibilityPolicy::defaultGrantedCapabilities()`. Guards were confirmed by sabotage: applying the policy to the records opens effective enablement, skipping the unusable-ledger check permits a write against invalid authority, committing when unchanged burns a generation, reusing the pre-commit snapshot instead of re-reading, and reporting a missing store as empty rather than unavailable; the controller's own discard-grants-when-not-Ready branch is defense in depth that cannot be observed through the real store, so the test pins the store-side contract directly instead of leaving an unverifiable guard. The full serial gate is `72/72` in 195.24s. No caller exists and `product_scope_policy` pins that the main window and extension center drive neither the controller nor the workflow and that the coordinator accepts no grant input, so every shipping record stays unenabled; the UI action, import, update, removal, encrypted backup, rollback, and recovery remain open, so `0.4` stays unchecked.
  - Shared secure storage authority slot follow-up: the enablement grant store had no platform backend, and giving it one meant either duplicating the review ledger's secure-storage adapter or extracting it, so `SecureStorageAuthoritySlotAdapter` now holds A/B slot selection, cache-bypassing reads, strict UTF-8 framing, bounded payloads, and the write-outcome trichotomy once, parameterized by a caller-supplied `SecureStorageAuthoritySlotScopes`, while the activation journal, review ledger, and enablement ledger keep only their own scopes and domain strings. `SecureStorageExtensionEnablementLedgerAdapter` anchors grant authority under `extensions/enablement-ledger-authority/slot-{a,b}/v1` with a frame schema and digest domain fully independent of the review ledger's, making domain separation a tested property at all three layers — codec, persistence, secure storage — because a shared format or scope would let a review payload be relocated into the grant position, turning "a human saw this content" into "a human asked to run this content". An unconfigured or collapsed scope set is refused as `secure-authority-slot-scopes-unconfigured` rather than falling back to a default location, since a facade that forgets its scopes must fail rather than silently share another subsystem's envelope and collapsing both slots onto one scope would degrade A/B publication back to the single copy whose torn write destroys the only HMAC key. Two hazards were found and fixed: the first cut silently dropped the activation journal's post-commit legacy-source removal, showing that byte-compatible framing is not sufficient evidence an extraction preserved behavior and every branch of the original including migration-only cleanup must be accounted for; and the first version of the focused test built its expected domains from literals declared inside the test file and compared the shared layer against those same literals, so it passed while never reading the adapters — swapping the enablement digest domain onto the review digest domain and drifting the review frame schema to `/0.2` both went undetected, so each adapter now exposes `authoritySlotScopes()` and the test compares those real values against its own independently rewritten copies of the persisted strings, and both previously silent sabotages now fail. The activation domain strings also existed in two places after the extraction, so `CompanionActivationAuthoritySlots::domain()` became the single source the adapter reads. `secure_storage_authority_slot_adapter` covers real scopes and domains against independent literals, pairwise scope distinctness including the legacy scope, accessor agreement, empty legacy scopes for both new subsystems, independent digest recomputation, cross-domain parse failure in both directions, and rejection of unconfigured scopes, collapsed slots, empty and oversized payloads, and a null read target. Guards were confirmed by sabotage: reusing the review slot scope or digest domain for enablement, drifting the review frame schema, adopting the review slot as a legacy scope, dropping legacy cleanup, hardcoding a subsystem scope in the shared layer, accepting an unconfigured scope set, and re-copying the activation domain constants instead of reading the single source. The full serial gate is `73/73` in 249.64s. This layer only moves bytes: it decides no trust, grants no enablement, and executes nothing, and `product_scope_policy` pins the absence of `effectiveEnabled` and `QProcess` in both the shared layer and the enablement facade, so every shipping record stays unenabled; a grant-producing UI action, import, update, removal, encrypted backup, rollback, and recovery remain open, so `0.4` stays unchecked.
...
- [ ] 17.3 Implement pinned context for files, selections, images, diagnostics, terminal excerpts, Git commits/diffs, artifacts, and child handoffs
  ...
  - Pinned diagnostic identity fix: `finishPinnedDiagnosticRaw` computed the `pin-diagnostic-<digest>` identity from six placeholders filled by a four-argument `arg()` plus three single-argument calls, exhausting every placeholder before `severity`. Severity therefore never entered the digest, so two diagnostics at the same project/root/reference/path/line/column differing only in severity collided on one identity and the second silently replaced the first in the pinned set. Now seven placeholders are filled by one seven-argument call. Because Qt reports format-string misuse only as a runtime stderr warning, a digest input can vanish with no test failing, which is a silent identity collision rather than a visible error; `agent_workbench_render` therefore installs a `QtMessageHandler` that counts every `QString::arg` warning across the whole render pass, chains to the previous handler, and fails at `AWB_UI_BASELINE` if the count is non-zero via the explicit `expect(condition, message, code)` overload rather than a new `setFailureStage` call, because `windows_packaging_policy` pins the exact ordered stage sequence in that main and a whole-pass invariant is not a stage. Verified in both directions and by the full serial gate `62/62`. No authority change.
