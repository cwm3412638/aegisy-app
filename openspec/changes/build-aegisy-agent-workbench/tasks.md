...
- [ ] 22.5 Verify sidecar and adapter hashes before launch and integrate their versions into updater compatibility checks
  ...
  - Partial signed-update foundation: `aegisy-update-artifact-set/0.1` verifies a bounded losslessly parsed Ed25519 envelope that binds release/channel/target, the full installer, target Runtime/adapter manifest, and one to 64 exact compatible installed artifact sets. The evaluation identity also binds the verification-key hash, selected channel, caller-supplied installed tuple, caller-supplied release-sequence high-water value, and evaluation time. Every result keeps download/install authority false; compatibility alone cannot start either operation. Focused CTest covers macOS/Windows positives, canonical payload/identity and an externally generated fixed Ed25519 vector, signature/field tampering, URL/path ambiguity, parser/size/depth/node boundaries, source bounds, complete installed-tuple matching/mismatch, clock and integer limits, and false authority. `UpdateManager` does not consume this contract, no trusted installed-tuple derivation or durable anti-deletion `{sequence, artifact-set identity, phase}` recovery record exists, Sparkle resume and current full-installer-only delta behavior are not closed, and WinSparkle 0.9.3 exposes no pre-download candidate veto. Keep `22.5` unchecked until a reviewed updater and signed clean-platform package evidence close those gates.
