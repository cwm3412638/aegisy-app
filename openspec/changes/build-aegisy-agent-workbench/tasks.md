...
- [ ] 0.3 Complete one-click profile apply/repair with target preview, per-tool isolation, backup, atomic verification, and truthful rollback status
  - Security prerequisite still open: current ToolManager config backups may contain credential-bearing CLI files. Encrypt backups from a SecureStorage-rooted key or separate credential material before claiming the one-click rollback path complete.
  - SecureStorage durability prerequisite: Windows credential saves now call `QSettings::sync()` and require `NoError` before caching success; credential removal on every platform clears the memory cache only after the platform backend confirms deletion, with Windows also requiring `sync()/NoError`. This prevents the future backup master key from treating an unflushed `setValue/remove` as durable success. Backups themselves remain plaintext until the encrypted backup-store slice lands, so `0.2` and `0.3` stay unchecked.
  - Encrypted-store foundation: `ConfigurationBackupStore` defines a path-free slot payload inside a strict canonical `aegisy-tool-config-backup` v2 AES-256-GCM manifest. It enforces exact tool/backup/time/count/AAD binding, 4 MiB per-file, 8 MiB aggregate, and 16 MiB manifest bounds; authenticates and fully validates slot/Base64/byte-count/SHA-256 data before returning any restore content; verifies the actual published manifest by bounded reread; and supports a resumable `manifest.v2.pending` migration from the exact legacy v1 layout. Its inventory now classifies `Empty/Ready/Unavailable/Invalid`, checks the complete locked root, migrates exact v1 records, authenticates every v2 record, returns only deterministic safe summaries plus domain-separated manifest identities, and preserves all evidence on any unknown/symlink/tamper/ambiguity. `removeVerified` rechecks that identity, authenticates before and after quarantine movement, and either removes the exact record or restores/preserves encrypted evidence. The injectable fake-provider test covers plaintext/path absence, round trip, wrong key/tool/ID, authenticated/unauthenticated tampering, bounds, inventory states/order, lock/key failure, legacy/incomplete migration, unknown evidence, identity replacement, tamper, and verified removal. This is not yet compiled into ToolManager or the application: production SecureStorage key provider, ToolManager/MainWindow integration, encrypted safety rollback, and prune orchestration remain open, so existing ToolManager backups are still plaintext and `0.2`/`0.3` remain unchecked.
