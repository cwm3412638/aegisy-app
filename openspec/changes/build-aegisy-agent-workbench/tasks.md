...
- [ ] 0.2 Define and implement the authenticated website-to-local configuration projection without exposing credential values outside secure storage
  ...
  - Management verification gap: focused contracts, unverified-account preflight, credential rebind, complete desktop build, and product-scope policy pass, but a trusted-origin fake-transport plus Qt race fixture is still required for positive read/create/update/delete/test, two-read handle rotation, held stale replies, uncertain mutation outcomes, create credential staging, and delete cleanup failure. Keep `0.2` unchecked until that runtime evidence and the cache/backup gates above pass.
