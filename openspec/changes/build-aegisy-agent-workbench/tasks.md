...
- [ ] 22.5 Verify sidecar and adapter hashes before launch and integrate their versions into updater compatibility checks
  ...
  - Partial signed-update foundation: `aegisy-update-artifact-set/0.1` verifies a bounded losslessly parsed Ed25519 envelope that binds release/channel/target, the full installer, target Runtime/adapter manifest, and one to 64 exact compatible source artifact sets. Production evaluation now requires an opaque `InstalledArtifactSetAuthority` derived from an exact-name adjacent signed receipt plus the verified Manifest, Runtime, and adapter bytes; the public scalar tuple exists only in the test build. Every candidate evaluation revalidates that local graph and binds its authority identity into the evaluation. Every result keeps download/install authority false; compatibility alone cannot start either operation. Focused CTest covers the earlier macOS/Windows, fixed-vector, parser, URL, tuple, replay, and false-authority matrix plus missing authority, bounded/oversized keys, receipt signature/key/sequence/expectation drift, Manifest/Runtime/adapter identity and content drift, separated paths, and link/hard-link rejection. `UpdateManager` does not consume this contract; caller-selected verification paths are not yet derived from or bound to the currently executing signed application; one shared receipt/candidate key has no reviewed rotation chain; no durable anti-deletion `{sequence, artifact-set identity, phase}` recovery record exists; Sparkle resume and current full-installer-only delta behavior are not closed; and WinSparkle 0.9.3 exposes no pre-download candidate veto. Keep `22.5` unchecked until a reviewed updater and signed clean-platform package evidence close those gates.
