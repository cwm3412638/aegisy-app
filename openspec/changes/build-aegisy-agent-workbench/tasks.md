...
- [ ] 22.5 Verify sidecar and adapter hashes before launch and integrate their versions into updater compatibility checks
  ...
  - Partial signed-update foundation: `aegisy-update-artifact-set/0.1` verifies a bounded losslessly parsed Ed25519 envelope that binds release/channel/target, the full installer, target Runtime/adapter manifest, and one to 64 exact compatible source artifact sets. Production evaluation now requires an opaque `InstalledArtifactSetAuthority`; its only production factory derives the fixed Windows application directory or macOS `AegisyClient.app/Contents/MacOS` layout from the executing `AegisyClient`, rejects caller-selected paths, verifies the exact-name adjacent signed receipt/Manifest/Runtime/adapter graph, and binds canonical directory identities plus the current application file identity, size, and SHA-256. Every candidate evaluation rederives and revalidates that graph. The scalar tuple and arbitrary-root factory exist only in the test build, and a production-shaped child fixture exercises the public fixed-layout path. Every result keeps download/install authority false. The internal `aegisy-update-progress-record/0.1` Store adds strict `{sequence, artifact-set identity, phase, revision}` continuity, exact retry, conflict/rollback/link/tamper rejection, atomic file replacement, and fixed-false authority, but it is not integrated and is not anti-deletion without an external secure anchor or cross-process CAS. `UpdateManager` still consumes neither contract; the current application hash is not bound by a verified outer package signature; one shared receipt/candidate key has no reviewed rotation chain; Sparkle resume/deltas remain open; and WinSparkle 0.9.3 exposes no pre-download candidate veto. Keep `22.5` unchecked until those gates and signed clean-platform package evidence are closed.
