...
- [ ] 0.3 Complete one-click profile apply/repair with target preview, per-tool isolation, backup, atomic verification, and truthful rollback status
  ...
  - Encrypted-store foundation: `ConfigurationBackupStore` defines a path-free slot payload inside a strict canonical `aegisy-tool-config-backup` v2 AES-256-GCM manifest. It enforces exact tool/backup/time/count/AAD binding, 4 MiB per-file, 8 MiB aggregate, and 16 MiB manifest bounds; authenticates and fully validates slot/Base64/byte-count/SHA-256 data before returning any restore content; verifies the actual published manifest by bounded reread; and supports a resumable `manifest.v2.pending` migration from the exact legacy v1 layout. Its key provider is injectable and the dedicated fake-provider test covers plaintext/path absence, round trip, wrong key/tool/ID, authenticated/unauthenticated tampering, bounds, legacy migration, interrupted migration, unknown evidence, and backend failure. This is not yet compiled into ToolManager or the application: production SecureStorage key provider, full inventory/state, verified prune/remove, UI integration, and rollback application remain open, so existing ToolManager backups are still plaintext and `0.2`/`0.3` remain unchecked.
