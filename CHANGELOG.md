...
- **Agent Workbench**: 全新的代理工作台界面（功能标志控制，默认禁用）
  - 完整的三栏响应式布局（会话列表、时间线、上下文面板）
  - 产品导航栏，支持 Chat、Work、Projects、Sessions、Extensions、Settings
  - 可调整大小的窗格，支持隐藏/显示和布局持久化
  - 命令面板（Cmd+K / Ctrl+K）快速访问功能
  - 原生菜单集成，支持键盘快捷键
  - 实时状态徽章（运行中、需要批准、失败、中断、后台）
  - 完整的主题系统（深色/浅色、高对比度、减少动画）
  - 完整的无障碍支持（ARIA、键盘导航、屏幕阅读器）

- **Timeline 和 Composer**: 对话时间线和消息输入
  - 6 种时间线项目类型（用户、代理、命令、错误、批准、使用情况）
  - 流式和完成状态渲染
  - 内联批准卡片，带风险等级指示器（高/中/低）
  - 结构化问题卡片，支持选项选择
  - Composer 输入区域，显示执行上下文（模式、项目、模型、权限）
  - 文件/图像/诊断附件预览
  - Context 面板显示选中文件和 Token 预算

### 安全增强

- 内容安全策略（CSP）强制执行，阻止所有外部资源
- 网络导航完全阻止，仅允许本地 qrc:// 资源
- JavaScript 沙箱化，无法访问本地文件或远程 URL
- 渲染器崩溃自动恢复
- 隔离的 WebEngine Profile，无缓存和 Cookie

### 技术改进

- 新增功能标志系统，支持 Internal/Preview/Beta/Stable 渠道
- 完整的测试覆盖（功能标志、安全、无障碍、窗口集成）
- 响应式设计，支持多种屏幕尺寸
...
- Section 11 (Workbench Host and Navigation): 100% 完成 (9/9 任务)
- Section 12 (Agent Timeline and Composer): UI 基础完成 (6 个部分任务)
- 总体进度: 31% (74/235 任务)
...
- 新增 AI 对话工作区，支持多会话、API Key 与模型切换、Markdown 回复、流式输出和停止生成。
- 消息支持复制、编辑后重发、直接重发和重新生成，并可复制完整 Markdown 对话。
- 增加实际/估算上下文 Token 展示；模型接口返回上限时同时显示上下文窗口。
