...
#[derive(Debug, Deserialize)]
struct SessionCompactionCheckpointReviseParams {
    session_id: String,
...
fn timeline_item_from_stored(stored: &StoredItem) -> Result<TimelineItem, String> {
    let content = stored
        ...
        .get("content")
        .and_then(Value::as_str)
        .ok_or_else(|| "sanitized Timeline item content is invalid".to_owned())?;
    let data = stored
        .payload
        .get("data")
    ...
    let item = TimelineItem {
        id: stored.item_id.clone(),
        kind: stored.item_kind.clone(),
    ...
    item.validate()
        .map_err(|_| "sanitized Timeline item is invalid".to_owned())?;
    Ok(item)
}
...
fn trace_hash_identity(domain: &str, fields: &[&str]) -> String {
    ...
    format!("sha256:{}", ContentHash::for_bytes(&input).sha256)
}

fn trace_error_class(class: &str) -> TraceErrorClass {
    match class {
        "protocol" => TraceErrorClass::Protocol,
        "provider" => TraceErrorClass::Provider,
        "transport" => TraceErrorClass::Transport,
        "timeout" => TraceErrorClass::Timeout,
        "sandbox" => TraceErrorClass::Sandbox,
        "policy" => TraceErrorClass::Policy,
        "tool" => TraceErrorClass::Tool,
        "storage" => TraceErrorClass::Storage,
        "workspace" => TraceErrorClass::Workspace,
        "git" => TraceErrorClass::Git,
        "budget" => TraceErrorClass::Budget,
        "adapter" | "runtime" => TraceErrorClass::Runtime,
        _ => TraceErrorClass::Unknown,
    }
}
...
fn codex_turn_trace_accumulator(
    ...
    let included_items = prepared_context
        .manifest
        .entries
        .iter()
        .filter(|entry| entry.included)
        .count();
    let excluded_items = prepared_context
        .manifest
        .entries
        .len()
        .saturating_sub(included_items);
    let context_item_count = prepared_context.manifest.entries.len();
    let context = TraceContextSummary {
        manifest_identity: manifest_identity.clone(),
        item_count: u32::try_from(context_item_count)
            .map_err(|_| "turn trace context item count is out of range".to_owned())?,
        included_items: u32::try_from(included_items)
            .map_err(|_| "turn trace included context count is out of range".to_owned())?,
        excluded_items: u32::try_from(excluded_items)
            .map_err(|_| "turn trace excluded context count is out of range".to_owned())?,
        bytes: u64::try_from(prepared_context.bytes)
            .map_err(|_| "turn trace context byte count is out of range".to_owned())?,
        omitted_fields: u32::try_from(excluded_items)
            .map_err(|_| "turn trace context omission count is out of range".to_owned())?,
        evidence_identity: manifest_identity,
    };
    ...
    let model = backend_info
        .provider
        .as_deref()
        .zip(backend_info.model.as_deref())
        ...
        .map(|(provider, model)| {
            let model_identity = trace_hash_identity("turn-trace-model", &[model]);
            let provider_identity = trace_hash_identity("turn-trace-provider", &[provider]);
            let reason_identity = trace_hash_identity(
                "turn-trace-session-model-binding",
                &[session_id, provider, model],
            );
            let evidence_identity = trace_hash_identity(
                "turn-trace-runtime-session-model-binding",
                &[session_id, turn_id, provider, model],
            );
            TraceModelMetadata {
                role: TraceModelRole::Agent,
                model_identity,
                provider_identity,
                reason: TraceModelReason::Continuation,
                reason_identity: Some(reason_identity),
                // The provider/model pair was observed when the Session binding
                // was created. At Turn start Runtime observes that binding; it
                // must not fabricate a fresh provider observation timestamp.
                evidence_source: TraceEvidenceSource::Runtime,
                evidence_identity,
            }
        });
...
fn trace_error_metadata(
    ...
    source_class: &str,
    retryable: bool,
    evidence_source: TraceEvidenceSource,
) -> TraceErrorMetadata {
    let retry = if retryable { "retryable" } else { "terminal" };
    let error_identity =
        trace_hash_identity("turn-trace-error", &[turn_id, class, source_class, retry]);
    let evidence_identity = trace_hash_identity(
        "turn-trace-error-evidence",
        &[turn_id, class, source_class, retry],
    );
    TraceErrorMetadata {
        error_identity,
        stable_class: trace_error_class(class),
...
fn trace_terminal_metadata(
    turn_id: &str,
    state: &str,
    evidence_source: TraceEvidenceSource,
) -> TraceTerminalMetadata {
    TraceTerminalMetadata {
        evidence_source,
        evidence_identity: trace_hash_identity("turn-trace-terminal-evidence", &[turn_id, state]),
    }
}
...
fn compaction_event_supersedes(
    event: &workbench_store::WorkbenchEvent,
    source: &session_compaction_store::CompactionCheckpointDescriptor,
) -> bool {
    let Some(supersedes) = event.payload.get("supersedes").and_then(Value::as_object) else {
        return false;
    };
    supersedes.get("checkpoint_id").and_then(Value::as_str) == Some(source.checkpoint_id.as_str())
        && supersedes.get("review_id").and_then(Value::as_str) == Some(source.review_id.as_str())
        && supersedes.get("object_reference").and_then(Value::as_str)
            == Some(source.object_reference.as_str())
}
...
impl Runtime {
    ...
    fn with_backend_and_store(
        ...
        let operation_reconciliations = workbench_store
            ...
            .collect();
        let event_sequencer = match workbench_store
            .as_ref()
            .map(WorkbenchStore::restore_public_timeline_sequencer)
        {
            Some(Ok(sequencer)) => sequencer,
            Some(Err(_)) => {
                backend = Backend::Unavailable(
                    "Public Timeline recovery failed; durable Runtime is unavailable".into(),
                );
                event_sequencer::EventSequencer::default()
            }
            None => event_sequencer::EventSequencer::default(),
        };
        Self {
            initialized: false,
            client_ready: false,
        ...
        F: FnMut(Value),
    {
        ...
        let messages = match request.method.as_str() {
            ...
            "session/compaction/checkpoint/revise" => {
                self.session_compaction_checkpoint_revise(request)
            }
    ...
    fn session_compaction_checkpoint_read(&self, request: Request) -> Vec<Value> {
        ...
        self.success_for(
            ...
            json!({
                ...
                "provider_compact_invoked": false,
                "original_event_history_authoritative": true,
                "supersedes": event.payload.get("supersedes").cloned(),
            }),
        )
    }

    fn session_compaction_checkpoint_revise(&mut self, request: Request) -> Vec<Value> {
        let params: SessionCompactionCheckpointReviseParams =
            match serde_json::from_value(request.params.clone()) {
                Ok(params) => params,
                Err(error) => {
                    return self.error_for(&request, -32602, format!("invalid params: {error}"))
        ...
        if params.source_checkpoint_id == params.checkpoint_id {
            return self.error_for(
                &request,
                -32602,
                "a compaction checkpoint revision requires a new checkpoint ID",
            );
        }
        if self.control.has_active_turn(&params.session_id) {
            return self.error_for(
        ...
        let (source_stored, source_descriptor) = match compaction_store
            .load_with_descriptor(&params.session_id, &params.source_checkpoint_id)
        {
            Ok(value) => value,
            Err(error) => return self.error_for(&request, -32113, error.message),
        };
        if source_stored.review.review_id != params.source_review_id {
            return self.error_for(
                &request,
                -32009,
                "source compaction review identity changed",
            );
        }
        if let Err(error) = find_compaction_checkpoint_event(
            workbench_store,
            &params.session_id,
            &source_descriptor,
            candidate.source_event_count,
        ) {
            return self.error_for(&request, -32115, error);
        }
        match compaction_store
            .load_optional_with_descriptor(&params.session_id, &params.checkpoint_id)
        {
            Ok(Some((stored, descriptor))) => {
                ...
                let event = match find_compaction_checkpoint_event(
                    ...
                    Ok(event) if compaction_event_supersedes(&event, &source_descriptor) => event,
                    Ok(_) => {
                        return self.error_for(
                            &request,
                            -32115,
                            "compaction checkpoint revision lineage is invalid",
                        )
                    }
                    Err(error) => return self.error_for(&request, -32115, error),
                };
                return self.success_for(
                    &request,
                    json!({
                        "schema_version": "session-compaction-checkpoint-revise-result/0.1",
                        "review": stored.review,
                        "descriptor": descriptor,
                        "event_sequence": event.sequence,
                        "idempotent_replay": true,
        ...
        let event = match workbench_store.append_session_compaction_checkpoint_revision_event(
            &descriptor,
            &review,
            &source_descriptor,
            &source_stored.review,
            now_ms(),
        ) {
            Ok(event) => event,
            Err(error) => return self.error_for(&request, -32113, error.message),
    ...
    fn persist_turn_with_timeline(
        ...
        let prepared = self
            .event_sequencer
            .prepare(observed_at_ms, session_id, turn_id, "turn.started", None)
            .map_err(|error| format!("cannot sequence turn start: {}", error.code()))?;
    ...
    fn persist_item_with_timeline(
        ...
        event_name: &str,
        item: &TimelineItem,
    ) -> Result<(TimelineItem, Value), String> {
        let observed_at_ms = now_ms();
        let mut request = StoredItemAppend {
            session_id: session_id.into(),
        ...
        let sanitized = match self.workbench_store.as_ref() {
            Some(store) => timeline_item_from_stored(
                &store
                    .preview_item_append(&request)
                    .map_err(|cause| cause.message)?,
            )?,
            None => item.clone(),
        };
        let prepared = self
            ...
            .map_err(|error| format!("cannot sequence persisted item: {}", error.code()))?;
        request.created_at_ms = prepared.envelope().timestamp_ms;
        let message = timeline_notification(prepared.envelope())?;
        if let Some(store) = self.workbench_store.as_mut() {
            store
                .append_item_with_public_event(request, prepared.envelope())
                .map_err(|cause| cause.message)?;
        }
        prepared
            .commit(&mut self.event_sequencer)
            .map_err(|error| format!("cannot commit persisted item: {}", error.code()))?;
        Ok((sanitized, message))
    }
    ...
    fn persist_item_with_command_artifact(
        ...
        let Some(artifact) = artifact else {
            return match public_event {
                Some(public_event) => {
                    store.append_item_with_public_event(item_append, public_event)
                }
                None => store.append_item(item_append),
            }
            .map(Some)
            .map_err(|cause| cause.message);
        };
        ...
        match public_event {
            Some(public_event) => store.append_item_with_durable_blob_and_public_event(
                item_append,
                blob,
                public_event,
            ),
            None => store.append_item_with_durable_blob(item_append, blob),
        }
    ...
    fn persist_turn_state_with_trace_and_timeline(
        ...
        let event_name = match state {
            ...
            "failed" => "turn.failed",
            "interrupted" | "cancelled" => "turn.interrupted",
            _ => return Err("turn terminal state is invalid".into()),
        };
        let terminal_item = item.as_ref().map(|item| StoredItemAppend {
            session_id: session_id.into(),
            turn_id: Some(turn_id.into()),
            item_id: item.id.clone(),
            item_kind: item.kind.clone(),
        ...
        let sanitized_item = match (self.workbench_store.as_ref(), terminal_item.as_ref()) {
            (Some(store), Some(item)) => Some(timeline_item_from_stored(
                &store
                    .preview_item_append(item)
                    .map_err(|cause| cause.message)?,
            )?),
            _ => item,
        };
        ...
        prepared
            .commit(&mut self.event_sequencer)
            .map_err(|error| format!("cannot commit turn terminal: {}", error.code()))?;
        if let (Some(state), Some(item)) = (self.sessions.get_mut(session_id), sanitized_item) {
                    ...
                    match event {
                        CodexEvent::TurnStarted { turn_id } => {
                            ...
                            let accumulator = match codex_turn_trace_accumulator(
                                ...
                                Err(error) => {
                                    persistence_error = Some(format!(
                                        "cannot prepare metadata-only turn trace: {error}"
                                    ));
                                    cancellation.request();
                                    return;
                                }
                            };
                            let turn_started_message = match self.persist_turn_with_timeline(
                                &params.session_id,
                                &turn_id,
                                &persistence_input,
                                &params.idempotency_key,
                                ...
                                Err(error) => {
                                    persistence_error =
                                        Some(format!("cannot persist turn: {error}"));
                                    cancellation.request();
                                    return;
                                }
                            };
                            ...
                            *turn_trace_accumulator
                            ...
                            let (persisted_user_item, user_item_message) =
                                match self.persist_item_with_timeline(
                                    &params.session_id,
                                    &turn_id,
                                    "item.completed",
                                    &user_item,
                                    ...
                                    Err(error) => {
                                        persistence_error =
                                            Some(format!("cannot persist user item: {error}"));
                                        cancellation.request();
                                        return;
                                    }
                                };
                        ...
                        CodexEvent::AgentCompleted {
                            ...
                            let (item, message) = match self.persist_item_with_timeline(
                                &params.session_id,
                                &turn_id,
                                "item.completed",
                                &item,
                                ...
                                Err(error) => {
                                    persistence_error =
                                        Some(format!("cannot persist agent item: {error}"));
                                    cancellation.request();
                                    return;
                                }
                            };
                        ...
                        CodexEvent::CommandUpdated {
                            ...
                            let mut item = command_timeline_item(
                                &command,
                                &lifecycle,
                                &command_environment,
                            ...
                            let mut persisted_event_message = None;
                            if lifecycle == "started" && self.workbench_store.is_some() {
                                ...
                                if let Some(store) = self.workbench_store.as_ref() {
                                    ...
                                    sanitized_item = match timeline_item_from_stored(&preview) {
                                        Ok(item) => item,
                                        Err(error) => {
                                            persistence_error = Some(error);
                                            cancellation.request();
                                ...
                                let prepared_event = match self.event_sequencer.prepare(
                                    item_append.created_at_ms,
                                    &params.session_id,
                                    &turn_id,
                                    "item.completed",
                                    Some(sanitized_item.clone()),
                                ) {
                                    Ok(prepared) => prepared,
                                    Err(error) => {
                                        sequencing_error = Some(error);
                                ...
                                if let Err(error) =
                                    prepared_event.commit(&mut self.event_sequencer)
                                {
                                    persistence_error = Some(format!(
                                        "cannot commit command Timeline item: {}",
                                        error.code()
                                    ));
                                    cancellation.request();
                                    return;
                                ...
                                item = sanitized_item;
                                persisted_event_message = Some(event_message);
                            ...
                            if let Some(message) = persisted_event_message {
                                emit(message);
                            } else {
                                ...
                                command_diagnostics
                            {
                                ...
                                let (diagnostic_item, message) =
                                    match self.persist_item_with_timeline(
                                        ...
                                        Err(error) => {
                                            persistence_error = Some(format!(
                                                "cannot persist diagnostic item: {error}"
                                            ));
                                            cancellation.request();
                                            return;
                                        }
                        ...
                        CodexEvent::TurnErrorObserved {
                            ...
                            let key = format!("error-observed:{turn_id}");
                            let count = metadata_update_counts.entry(key.clone()).or_default();
                            if *count < MAX_TURN_METADATA_UPDATES_PER_KIND {
                                *count += 1;
                                let mut data = runtime_error_data_with_provider(
                                    &message,
                                    provider_error.as_ref(),
                                );
                                data["terminal"] = Value::Bool(false);
                                data["will_retry"] = Value::Bool(will_retry);
                                let item = TimelineItem {
                                    id: self.allocate_id("error-observed"),
                                    kind: "error".into(),
                                    role: "system".into(),
                                    state: "updated".into(),
                        ...
                        CodexEvent::TurnCompleted { turn_id } => {
                            let terminal_at_ms = match self.event_sequencer
                                .normalized_timestamp(now_ms(), &params.session_id)
                            {
                            ...
                            *turn_trace_accumulator
                            ...
                            let timeline_message = match self
                                .persist_turn_state_with_trace_and_timeline(
                                &params.session_id,
                                &pending.turn_id,
                                pending.state,
                        ...
                        CodexEvent::TurnInterrupted { turn_id } => {
                            ...
                            let trace = match accumulator.clone().finalize_interrupted(
                                ...
                                Err(error) => {
                                    persistence_error = Some(format!(
                                        "cannot finalize interrupted turn trace: {}: {}",
                                        error.code, error.message
                                    ));
                                    return;
                                }
                            ...
                            *turn_trace_accumulator
                            ...
                            let pending = pending_terminal
                                .as_ref()
                                .expect("interrupted terminal trace was prepared");
                            let timeline_message = match self
                                .persist_turn_state_with_trace_and_timeline(
                                &params.session_id,
                                &pending.turn_id,
                                ...
                                Err(error) => {
                                    persistence_error = Some(format!(
                                        "cannot persist interrupted turn: {error}"
                                    ));
                                    cancellation.request();
                                    return;
                                }
                        ...
                        CodexEvent::TurnFailed {
                            ...
                            let (class, source_class, retryable, evidence_source) = provider_error
                                .as_ref()
                                .map(|error| {
                                    (
                                        error.class,
                            ...
                            let trace = match accumulator.clone().finalize_failed(
                                ...
                                trace_terminal_metadata(&turn_id, "failed", evidence_source),
                            ) {
                                Ok(trace) => trace,
                                Err(error) => {
                                    persistence_error = Some(format!(
                                        "cannot finalize failed turn trace: {}: {}",
                                        error.code, error.message
                                    ));
                                    return;
                                }
                            ...
                            *turn_trace_accumulator
                            ...
                            let pending = pending_terminal
                                .as_ref()
                                .expect("failed terminal trace was prepared");
                            let timeline_message = match self
                                .persist_turn_state_with_trace_and_timeline(
                                &params.session_id,
                                &pending.turn_id,
            ...
            let terminal_result = match (started_turn_id.as_deref(), accumulator) {
                (Some(turn_id), Some(accumulator)) => {
                    let terminal_at_ms = self
                        ...
                        .map_err(|error| {
                            format!(
                                "cannot normalize persistence-failure timestamp: {}",
                                error.code()
                            )
                        });
                    terminal_at_ms.and_then(|terminal_at_ms| {
                        accumulator
                            .finalize_failed(
                                terminal_at_ms,
                                trace_error_metadata(
                            ...
                            .map_err(|error| {
                                format!(
                                    "cannot finalize persistence-failure trace: {}: {}",
                                    error.code, error.message
                                )
                            })
                            .and_then(|trace| {
        ...
        if let Err(error) = result {
            if started {
                    ...
                    .normalized_timestamp(now_ms(), &params.session_id)
                {
                    ...
                    Err(cause) => {
                        self.backend = Backend::Unavailable(format!(
                            "Timeline timestamp normalization is unavailable: {}",
                            cause.code()
                        ));
                        self.shutdown = true;
                        return;
                ...
                let terminal_result = turn_trace_accumulator
                    ...
                    .and_then(|accumulator| {
                        accumulator
                            ...
                            .map_err(|cause| {
                                format!(
                                    "cannot finalize adapter-failure trace: {}: {}",
                                    cause.code, cause.message
                                )
                            })
                    })
    ...
    fn timeline_sync(&self, request: Request) -> Vec<Value> {
        let params: TimelineSyncParams = match serde_json::from_value(request.params.clone()) {
            Ok(params) => params,
            Err(error) => {
                return self.error_for(&request, -32602, format!("invalid params: {error}"))
            }
        ...
        let Some(store) = self.workbench_store.as_ref() else {
            return self.error_for(&request, -32120, "durable Timeline replay is unavailable");
        };
        let after = public_timeline_journal::TimelineWatermark {
            sequence: params.after.sequence,
            event_id: params.after.event_id.clone(),
        };
        let watermark =
            params
                .watermark
                .as_ref()
                .map(|anchor| public_timeline_journal::TimelineWatermark {
                    sequence: anchor.sequence,
                    event_id: anchor.event_id.clone(),
                });
        let limit = match usize::try_from(params.limit) {
            Ok(limit) => limit,
            Err(_) => return self.error_for(&request, -32602, "timeline sync limit is invalid"),
        };
        let stored = match store.sync_public_timeline_from_anchor(
            &params.session_id,
            after,
            watermark,
            limit,
        ...
        let page = AapTimelineSyncPage {
            ...
            after: TimelineAnchor {
                sequence: stored.after_sequence,
                event_id: params.after.event_id.clone(),
            },
            watermark: TimelineAnchor {
                sequence: stored.watermark.sequence,
                event_id: stored.watermark.event_id,
            },
            next_after: stored.events.last().and_then(|event| {
                (!stored.complete).then(|| TimelineAnchor {
                    sequence: event.sequence,
        ...
        if page.validate_for_request(&params).is_err() {
            return self.error_for(
                &request,
                -32147,
                "durable Timeline sync response failed validation",
            );
        }
        match serde_json::to_value(page) {
            Ok(result) => self.success_for(&request, result),
            Err(_) => self.error_for(
                &request,
                -32147,
                "durable Timeline sync response serialization failed",
            ),
        }
    }
    ...
    fn event(
        ...
        let prepared = self
            .event_sequencer
            .prepare(now_ms(), session_id, turn_id, event, item)?;
        let message = serde_json::to_value(Notification {
            jsonrpc: JSONRPC_VERSION,
            method: "event",
            params: prepared.envelope().clone(),
        })
        .map_err(|_| {
            event_sequencer::SequenceError::Rejected("notification-serialization-failed")
        })?;
        if let Some(store) = self.workbench_store.as_mut() {
            store
                .append_public_timeline_event(prepared.envelope())
                .map_err(|_| {
                    event_sequencer::SequenceError::Rejected("timeline-journal-persistence-failed")
                })?;
        }
        prepared.commit(&mut self.event_sequencer)?;
        Ok(message)
    }
...
mod timeline_event_runtime_tests {
    ...
    fn persisted_turn_start_uses_the_sequencer_timestamp_after_clock_rollback() {
        let unique = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let root = std::env::temp_dir().join(format!("aegisy-timeline-clock-rollback-{unique}"));
        std::fs::create_dir_all(&root).unwrap();
        ...
        runtime
            ...
            .create_turn_with_public_event(
                StoredTurnCreate {
                    turn_id: "clock-prior-turn".into(),
                    session_id: "clock-session".into(),
                    idempotency_key: Some("clock-prior-key".into()),
                    input_hash: ContentHash::for_bytes(b"prior"),
                    created_at_ms: future_timestamp,
                },
                prior.envelope(),
            )
            .unwrap();
        prior.commit(&mut runtime.event_sequencer).unwrap();
        let prior_completed = runtime
            .event_sequencer
            .prepare(
                future_timestamp,
                "clock-session",
        ...
        runtime
            ...
            .as_mut()
            .unwrap()
            .append_public_timeline_event(prior_completed.envelope())
            .unwrap();
        prior_completed
        ...
        let stored = runtime
            ...
            .as_ref()
            .unwrap()
            .load_turn("clock-next-turn")
            .unwrap();
        assert_eq!(stored.created_at_ms, future_timestamp);
        assert_eq!(stored.updated_at_ms, future_timestamp);
    ...
    #[test]
    fn durable_runtime_restores_timeline_sequence_and_rejects_forged_sync_anchor() {
        let data_root = std::env::temp_dir().join(format!(
            "aegisy-runtime-timeline-sync-{}-{}",
            std::process::id(),
            now_ms()
        ));
        ...
        runtime
            ...
            .unwrap()
            .create_session(StoredSessionCreate {
                session_id: "runtime-timeline-session".into(),
                project_id: None,
                mode: StoredSessionMode::Chat,
                title: "Runtime Timeline".into(),
                parent_session_id: None,
                lineage_kind: StoredSessionLineage::New,
                environment_identity: None,
                created_at_ms: 1,
        ...
        let first = runtime
            .event(
                ...
                Some("runtime-turn-one"),
                "turn.started",
                None,
            )
            .unwrap();
        let first_event_id = first["params"]["event_id"].as_str().unwrap().to_owned();
        let second = runtime
            .event(
                "runtime-timeline-session",
                Some("runtime-turn-one"),
        ...
        let sync_request: Request = serde_json::from_value(json!({
            ...
            "params": {
                ...
                "after": { "sequence": 1, "event_id": first_event_id },
                "watermark": null,
                "limit": 200
            }
        }))
        .unwrap();
        let page = reopened.timeline_sync(sync_request);
        assert_eq!(page[0]["result"]["watermark"]["sequence"], 3);
        assert_eq!(page[0]["result"]["events"].as_array().unwrap().len(), 2);
        assert_eq!(page[0]["result"]["complete"], true);

        let forged: Request = serde_json::from_value(json!({
            "jsonrpc": "2.0",
            "id": "timeline-sync-forged",
            "method": "timeline/sync",
            "params": {
                "session_id": "runtime-timeline-session",
                "after": {
...
mod turn_cancel_tests {
    ...
    fn runtime_error_classification_is_bounded_redacted_and_retry_aware() {
        ...
        for (message, class) in [
            ...
            (
                "Codex App Server protocol error: invalid turn status",
                "protocol",
            ),
    ...
    fn runtime_error_data_preserves_structured_provider_metadata() {
        ...
        assert_eq!(
            runtime_error_content_with_provider(secret, None),
            "Codex turn failed"
        );
    }
