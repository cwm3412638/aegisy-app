...
// embedded schemas are split into adjacent concatenated raw literals. Chunks
// are split only at ASCII boundaries to keep every literal valid UTF-8.
const cppChunkedRawSchema = (text, variable) => {
    const chunks = [];
    for (let offset = 0; offset < text.length;) {
        let end = Math.min(offset + 7000, text.length);
        while (end < text.length && text.charCodeAt(end) > 0x7f) end += 1;
    ...
    const literal = chunks.map((chunk) => `R"AAPSCHEMA(${chunk})AAPSCHEMA"`).join("\n    ");
    return `static const char ${variable}[] =\n    ${literal};`;
};
...
namespace aegisy::aap::transport_generated {
namespace {
${cppChunkedRawSchema(JSON.stringify(schema), "transportSchemaJson")}

${cppChunkedRawSchema(JSON.stringify(genericEnvelopeSchema), "genericEnvelopeSchemaJson")}
...
const transport_runtime::TransportSchemaRuntime *transportRuntime(QString *error)
{
    ...
    static const std::unique_ptr<transport_runtime::TransportSchemaRuntime> runtime =
        transport_runtime::TransportSchemaRuntime::fromRawSchema(
            QByteArray(transportSchemaJson, sizeof(transportSchemaJson) - 1),
            &initializationError);
...
const transport_runtime::TransportSchemaRuntime *genericTransportRuntime(QString *error)
{
    ...
    static const std::unique_ptr<transport_runtime::TransportSchemaRuntime> runtime =
        transport_runtime::TransportSchemaRuntime::fromRawSchema(
            QByteArray(genericEnvelopeSchemaJson, sizeof(genericEnvelopeSchemaJson) - 1),
            &initializationError);
    if (!runtime && error) *error = initializationError;
    return runtime.get();
}
